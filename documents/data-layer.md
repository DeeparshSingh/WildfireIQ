...
| | |
|---|---|
| **What it shows** | Our AI's estimate of wildfire risk today, drawn as coloured hexagons (Low / Moderate / High / Extreme) across four modelled regions: Thompson-Okanagan (Kamloops), Central Okanagan (Kelowna), Lower Mainland (Vancouver), and Prince George. A selector card lets you turn each region on or off. |
| **Where the data comes from** | An in-house machine-learning model trained on **23 years** of BC Wildfire Service fire records (96,356 province-wide incidents) plus ERA5 reanalysis weather sampled at each region's anchor city. |
...
| **How it's computed** | Two ingredients multiplied together: **(a)** a region-wide probability that *at least one* fire ignites there today, predicted from that region's own weather + FWI codes + its long-run fire-day rate; **(b)** each hexagon's historical fire frequency. Cell risk = regional probability × that cell's normalised historical density, then bucketed into the four classes. Every hexagon belongs to exactly one region. |
| **Accuracy** | Trained on 1999–2021, tuned on 2022, and **tested on 2023 — a year the model never saw**. Scored separately per region on that unseen year: **Thompson-Okanagan 0.72**, **Prince George 0.61**, **Central Okanagan 0.51**, **Lower Mainland 0.29**, against an FWI-threshold baseline of **0.37**. (PR-AUC measures how well it ranks true fire days above non-fire days; higher is better.) |
| **Honest limits** | Each region reads its own weather, but *within* a region the hexagon-to-hexagon difference comes from fire history rather than separate local weather, so two neighbouring hexagons differ only by their past. The Lower Mainland is a low-event area: real wildfires there are rare, which is both why it reads Low and why it is the hardest region to score. Alongside every hexagon we show the deterministic government **CFFDRS Fire Danger** class for that region so you can sanity-check the AI against the standard. |

### How the model actually works (step by step)

1. **Training data.** For every day from 1999 to 2021, in each of the four regions, we built a row of 42 features: that region's weather (temperature, humidity, wind, rain, vapour-pressure deficit), the six FWI codes, lagged and rolling versions (7-day and 30-day windows), drought signals (days since rain), calendar terms (day-of-year), and the region's long-run fire-day rate. The label is simply: did a fire ignite in that region that day?
2. **Algorithm.** One **LightGBM** gradient-boosted tree classifier pooled across all four regions (33,576 training region-days). Pooling gives the model four times the weather-to-fire evidence to learn from, and the base-rate feature keeps a wet coastal region from being scored like a dry Interior one. It outputs a calibrated probability between 0 and 1.
...
4. **Validation discipline.** We never let the model see 2022 or 2023 during training, and the base-rate feature is computed from 2021 and earlier only, so no future information leaks backwards. Scores are reported per region rather than as one pooled average, so pooling cannot hide a regression in a single area.
5. **Per-cell step.** Each region's probability is scaled by each H3 hexagon's historical fire count (square-root-normalised so a few extreme cells don't dominate), then bucketed Low/Moderate/High/Extreme.
6. **Region badge.** The headline class shown on a region's card is the highest class that covers at least 15% of its hexagons, so the badge always agrees with what the map is drawing.
