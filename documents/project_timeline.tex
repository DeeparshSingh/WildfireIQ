...
\begin{quote}
\small\itshape Status note, September 2026: this is the plan as written at the
start of the grant, kept for provenance. Every feature and both models shipped;
a few technology choices changed during the build. The risk model grew from one
region to four; DuckDB and ONNX Runtime were scoped here but are not in the
delivered stack, because pandas reads the Parquet files fast enough at this
scale and nothing loaded the ONNX artifact at runtime; MapLibre GL was not
needed; the backend runs on Python 3.12. For what was actually built, read
\texttt{WildfireIQ-Report.tex} and the repository README.
\end{quote}
...
\begin{itemize}
    \item \textbf{Documented API quotas:} Every external dependency is used within its provider's documented quota, with the required keys entered in the app rather than committed to the repository.
...
\vspace{0.5cm}
\vspace{0.3cm}
\noindent\textbf{As built (September 2026).} This timeline is the plan as approved
in May 2026 and is kept unchanged for the record. The delivered platform differs
in these respects: the risk model covers four regions (Thompson-Okanagan, Central
Okanagan, Lower Mainland, Prince George) rather than one, with 523 H3 hexagons;
the ingestion layer settled at 17 jobs and uses Parquet plus SQLite only, without
DuckDB; the models are served in-process by LightGBM and are not exported to ONNX;
the 3D globe uses CesiumJS alone, without MapLibre or GSAP; and a fifth module not
in the plan, a tool-using assistant over the platform's own data (GLM~5.3~Flash via
OpenRouter), was added after the four planned features were complete.
\vspace{0.3cm}
