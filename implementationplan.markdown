...
| **1** | Data ingestion + ETL pipeline | 1-2 sittings | DuckDB + SQLite populated with 25 yrs of BC fires, daily Kamloops weather, AQ, FWI, evac feed; backend exposes raw `/api/*` endpoints |
| **2** | 3D Cesium globe + Wildfire Risk Map | 2-3 sittings | Live 3D globe over Thompson-Okanagan with current fires, FIRMS hotspots, animated camera intro, layer toggles, hover/click info panels |
| **3** | ML models — wildfire risk classifier + 48-hr AQ forecaster | 2 sittings | Two trained models, validation reports, model-cards, served via `/api/risk` and `/api/aq/forecast` |
| **4** | Air Quality Monitor feature | 1-2 sittings | Full AQ dashboard: current AQHI, 48-hr forecast chart, smoke event calendar, health guidance |
