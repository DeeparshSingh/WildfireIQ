...
Target: P(at least one fire ignites in a region today | that region's
weather + FWI + temporal features + its base-rate prior).

One model is pooled across every region in `constants.REGIONS`, so it learns
the universal weather-to-fire relationship while `region_fire_rate` carries
each area's very different base rate.

Train: 1999-2021.  Val: 2022.  Test: 2023.  Per-region held-out scores are
written to metrics.json so no single region can regress unnoticed.

At inference time the region probability is multiplied by each H3 r=5 cell's
`weight` (sqrt-normalized historical fire count) to produce a per-cell risk
score, which is then bucketed:
...
def main() -> None:
    ...
    if "region" in test.columns:
        ...
        for key, grp in ev.groupby("region"):
            ...
            per_region[str(key)] = {
                ...
                "n_fire_days": n_fire,
                "pr_auc": (
                    float(average_precision_score(grp["_y"], grp["_p"])) if n_fire > 0 else None
                ),
    ...
    metrics["baseline_fwi_thresh_pr_auc"] = float(average_precision_score(y_test, fwi_threshold))
    ...
    metrics["feature_importance"] = sorted(
        [{"feature": f, "gain": float(g)} for f, g in zip(FEATURE_COLS, importances, strict=True)],
        key=lambda x: -x["gain"],
    )[:20]
